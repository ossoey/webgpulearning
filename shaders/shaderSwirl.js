...
fn applyBlendedSwirls(uv: vec2<f32>, time: f32) -> vec2<f32> {
  ...
  let baseStrength = 1.0 + 0.9 * sin(time * 0.8);
...
fn fs_main(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
  ...
  // let newTheta = theta + time.value * swirlStrength * (1.0 - r);
  ...
  ///////
   ...
   let distortedUV = applyBlendedSwirls(uv, time.value);

  
  // return textureSample(myTexture, mySampler, uv);

    let baseColor = textureSample(myTexture, mySampler, distortedUV);

    let totalColumns = 15.0;

     let totalRows = 150.0;

    let columnIndex = u32(floor(distortedUV.x * totalColumns));

     let rowIndex = u32(floor(distortedUV.y * totalRows )); 

    var finalColor = baseColor;

    if ((columnIndex % 4u) == 0u) {
        let blue = vec4<f32>(0.6, 0.8, 1.0, 2.2);
        finalColor = mix(baseColor, blue, 0.5);
    } else if ((columnIndex % 7u) == 0u) {
        let blue = vec4<f32>(0.0, 2.8, 3.0, 0.02);
        finalColor = mix(baseColor, blue, 0.5);
    }
